...
<div class="container py-4 mb-5">
    ...
    <div class="mb-2 mx-auto" style="max-width: 1200px;">
        ...
        <div class="d-flex flex-wrap justify-content-center justify-content-md-start gap-4">
            {{-- Forzamos a que muestre 5 con ->take(5) --}}
            @foreach($librosPopulares->take(5) as $libroPopular)
            <div style="width: 210px; flex-shrink: 0;">
                <a href="{{ route('libros.show', $libroPopular->id ?? 1) }}" class="text-decoration-none d-block h-100">
                    <div class="card h-100 book-card border-0">
                        <div class="book-cover-container position-relative skeleton-bg">
                            {{-- Insignia TOP eliminada --}}

                            @if($libroPopular->portada)
                            <img src="{{ $libroPopular->portada }}" alt="{{ $libroPopular->titulo }}" class="book-cover" onload="this.parentElement.classList.remove('skeleton-bg'); this.style.opacity=1;" style="opacity: 0;">
                            @else
                        ...
                        <div class="card-body p-3 d-flex flex-column">
                            <span class="category-tag mb-1">{{ $libroPopular->categoria->nombre ?? 'General' }}</span>
                            <h6 class="book-title mb-1 text-truncate">{{ $libroPopular->titulo }}</h6>
                            <p class="book-author mb-3 text-truncate" style="font-size: 0.8rem;">{{ $libroPopular->autor }}</p>

                            <div class="mt-auto d-flex justify-content-between align-items-center pt-2 border-top-subtle">
                                <div class="status-indicator">
                                    @if($libroPopular->copias_totales > 0)
    ...
    <div class="mb-2 mx-auto position-relative" style="max-width: 1200px;">
        ...
        <div class="d-flex overflow-auto pb-4 gap-4 css-scrollbar-hide snap-scroll-container" id="scroll-cat-{{ $catEscaparate->id }}" style="scroll-behavior: smooth;">
            ...
            <div class="snap-item" style="min-width: 210px; max-width: 210px; flex-shrink: 0;">
                <a href="{{ route('libros.show', $libroCat->id ?? 1) }}" class="text-decoration-none d-block h-100">
                    <div class="card h-100 book-card border-0">
                        <div class="book-cover-container skeleton-bg">
                            @if($libroCat->portada)
                            <img src="{{ $libroCat->portada }}" alt="{{ $libroCat->titulo }}" class="book-cover" onload="this.parentElement.classList.remove('skeleton-bg'); this.style.opacity=1;" style="opacity: 0;">
                            @else
                            <div class="book-cover d-flex justify-content-center align-items-center" style="background-color: var(--bg-light);">
                                <i class="bi bi-book fs-1" style="color: var(--secondary-light); opacity: 0.5;"></i>
                            </div>
                        ...
                        <div class="card-body p-3 d-flex flex-column">
                            <h6 class="book-title mb-1 text-truncate mt-1">{{ $libroCat->titulo }}</h6>
                            <p class="book-author mb-3 text-truncate" style="font-size: 0.8rem;">{{ $libroCat->autor }}</p>

                            <div class="mt-auto d-flex justify-content-between align-items-center pt-2 border-top-subtle">
                                <div class="status-indicator">
                                    @if($libroCat->copias_totales > 0)
                                    <span class="status-dot available"></span> <span class="status-text" style="font-size: 0.7rem;">Disponible</span>
                                    @else
                                    <span class="status-dot unavailable"></span> <span class="status-text text-muted" style="font-size: 0.7rem;">Agotado</span>
                                    @endif
                                </div>
                                <span class="btn btn-sm btn-outline-details rounded-pill py-1 px-3" style="font-size: 0.75rem; font-weight: bold;">Detalles</span>
                            </div>
    ...
    <div class="row row-cols-2 row-cols-md-3 row-cols-lg-4 justify-content-center mx-auto g-4" style="max-width: 1200px;">
        ...
        <div class="col">
            <div class="card h-100 book-card border-0">
                <div class="book-cover-container skeleton-bg">
                    @if($libro->portada)
                    <img src="{{ $libro->portada }}" alt="Portada de {{ $libro->titulo }}" class="book-cover" onload="this.parentElement.classList.remove('skeleton-bg'); this.style.opacity=1;" style="opacity: 0;">
                    @else
                    <div class="book-cover d-flex justify-content-center align-items-center" style="background-color: var(--bg-light);">
                        <i class="bi bi-book fs-1" style="color: var(--secondary-light); opacity: 0.5;"></i>
                    </div>
                ...
                <div class="card-body d-flex flex-column p-3">
                    ...
                    <p class="book-author mb-3 text-truncate">{{ $libro->autor }}</p>
                    <div class="mt-auto d-flex justify-content-between align-items-center pt-3 border-top-subtle">
                        <div class="status-indicator">
                            @if($libro->copias_totales > 0)
                            <span class="status-dot available"></span> <span class="status-text d-none d-sm-inline">Disponible</span>
                            @else
                            <span class="status-dot unavailable"></span> <span class="status-text text-muted d-none d-sm-inline">Agotado</span>
                            @endif
                        </div>
                        <a href="{{ route('libros.show', $libro->id ?? 1) }}" class="btn btn-sm btn-outline-details rounded-pill px-3 fw-bold">Detalles</a>
                    </div>
                </div>
            </div>
        </div>
